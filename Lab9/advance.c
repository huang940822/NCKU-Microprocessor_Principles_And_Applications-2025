...
void variable_register_changed(int value) {  // value: 0 ~ 1023
    ...
    // 1. 縮放數值 (0~1023 -> 0~15)
    // 1024 / 16 = 64
    int scaled_val = value / 64;
    
    // 2. 判斷上升或下降
    if (scaled_val > prev_value) {
        // [上升] 顯示奇數 (1, 3, 5...)
        if (scaled_val & 1) { // 檢查最低位是否為 1 (奇數)
            set_LED(scaled_val);
        }
    } 
    else if (scaled_val < prev_value) {
        // [下降] 顯示偶數 (0, 2, 4...)
        if (!(scaled_val & 1)) { // 檢查最低位是否為 0 (偶數)
            set_LED(scaled_val);
        }
    }
    
    // 3. 更新前次數值
    prev_value = scaled_val;
