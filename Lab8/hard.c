...
// ==========================================
// ★★★ 你的邏輯變數 ★★★
bool is_running = false; // 控制是否掃描
int angle = -90;         // 目前角度
int direction = 1;       // 1: 增加, -1: 減少
int step = 2;            // 每次移動2度 (控制掃描速度)
// ==========================================
...
void main() {
    ...
    while (1) {
        ...
        if(is_running){
            ...
            // 4. ★設定 LED 亮度 (RC1/CCP2)★
            // 邏輯：角度 (-90 ~ 90) 對應到 亮度 (0 ~ 1023)
            // 公式：(angle + 90) * 1023 / 180  就是0-180用比例對應到0-1023
            // 使用 long 避免計算過程溢位
            unsigned int LED_PWM = (unsigned int)((long)(angle + 90) * 1023 / 180);

            set_LED_analog(LED_PWM);

            // 5. 掃描速度控制
            __delay_ms(10); // 調整這裡可以變快或變慢
        }
