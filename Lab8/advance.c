...
void Initialize(void) {
    ...
    // Configure I/O ports
    //TRISA &= 0xF1;  // 1111 0001 Set RA1-RA3 as outputs for LED
    TRISB = 1;      // RB0 as input for button
    TRISC = 0;      // PORTC as output for servo
    TRISD &= 0x0F; // 0000 1111 Set RD7-RD4 as outputs for LED
    LATD = 0x00;   // 預設全暗
    LATC = 0;       // Clear PORTC
...
// ==========================================
// ★★★ 你的邏輯變數 ★★★
bool run_motor_flag = false; // 告訴 main 要不要動
int current_angle = -90;     // 目前角度
int direction = 1;           // 1: 增加角度, -1: 減少角度
int step_angle = 135;        // 每次增加135度 (對應原本的 PWM 44 step)
// ==========================================
void button_pressed() {
...
void main() {
    ...
    while (1) {
        // Do sth in main
        if (run_motor_flag) {
            //執行一次interrupt後，清除旗標，避免重複執行
            run_motor_flag = false;
            
            // 1. 計算下一個目標角度
            int next_angle = current_angle + (direction * step_angle);
            
            // 2. 邊界判斷 (Clamp & Reverse)
            if (next_angle >= 90) {
                next_angle = 90;   // 超過就停在 90
                direction = -1;    // 下次改成減少
            } 
            else if (next_angle <= -90) {
                next_angle = -90;  // 低於就停在 -90
                direction = 1;     // 下次改成增加
            }
            
            // 3. 呼叫 Template 函式執行轉動
            // 這函式已經包含了 "慢慢轉 (smooth)" 的功能
