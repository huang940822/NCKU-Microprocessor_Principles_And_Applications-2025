...
// =================== TODO ========================
// 題目邏輯變數
int counter = 0;       // 計數器 (0 -> 無限大)
int old_len = 0;       // 記錄上一次輸出的字串長度 (用來決定 Backspace 次數)
char str_buf[20];      // 字串緩衝區
void button_pressed() {
    // Do sth when the button is pressed
    /* Example:
     * set_LED(get_LED() + 1);
     */
    // 1. 刪除舊數值 (In-Place Update) 
    // 根據上次印出的長度，送出對應數量的 Backspace
    ...
    // 3. 更新 UART (Decimal) [cite: 8]
    sprintf(str_buf, "%d", counter); // 轉成十進位字串
    printf("%s", str_buf);           // 印出
    
    // 4. 更新長度記錄
    old_len = strlen(str_buf);
    
    // 5. 更新 LED (Modulo 16, Binary) 
    set_LED(counter % 16);
}
