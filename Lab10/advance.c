...
void Initialize(void) {
    ...
    //重要 要同時在馬達跟LED用PWM
    CCP1CONbits.CCP1M = 0b1100;  //設定 CCP1 PWM mode
    CCP2CONbits.CCP2M = 0b1100; // 設定 CCP2 為 PWM 模式
    PR2 = 0x9b;                  // Set PWM period
    
    TRISCbits.TRISC1 = 0;  // RC1 -> CCP2 燈泡輸出
    TRISCbits.TRISC2 = 0;  // RC2 -> CCP1 (servo)
    
    // [重要] Timer2 設定 (覆蓋上面的 PWM 設定，改為計時用)
    // 目標：50ms 中斷一次
    // Fosc=4MHz, Tcy=1us
    // PR2=194, Pre=16, Post=16 => (194+1)*16*16*1us = 49920us ~= 50ms
    T2CON = 0b01111111; // Post:1:16, On, Pre:1:16
    PR2 = 194;
    //*** 如果馬達要轉同時LED要閃爍，就用PR2 = 0x9b;上面這坨刪掉，然後PR2 = 0x9b=2.5ms PR2 = 194=50ms 所以所有tick要乘20倍
    // Configure I/O port
    TRISB = 1;      // RB0 as input for button
    TRISC = 0;      // PORTC as output for servo
    // [重要] LED I/O Setting: RD7-RD4 as Output
    TRISD &= 0x0F; 
    LATD = 0x00;   // 預設全暗
    LATC = 0;       // Clear PORTC
...
// [題目變數]
volatile int target_ticks = 20; // 預設 1.0s (20 * 50ms)
volatile int current_ticks = 0; // 目前累積的 tick
int led_state = 0;
void keyboard_input(char *str) {  // get line from keyboard: this function will be called after you click enter
     ...
     */
    float val = atof(str); // 轉成浮點數
    
    if (val >= 0.1 && val <= 1.0) {
        // 50ms time base -> 0.05s
        target_ticks = (int)(val * 20); // val / 0.05 等於 val * 20 
        printf("\r\nSet delay: %.1f s\r\n", val);
    } else {
        printf("\r\nInvalid input\r\n");
    }
...
void main() {
    ...
    led_state = 1;
    set_LED(led_state); // 開燈 (使用 set_LED)
    while (1) {
        // Do sth in main
        // Timer2 計時 (Polling) - 避免使用 delay 函式卡住
        if (PIR1bits.TMR2IF) {
            PIR1bits.TMR2IF = 0; // 清除旗標
